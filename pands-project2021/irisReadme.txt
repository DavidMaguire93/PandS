...
image would be given a title. Titles for each individual plot were automatically generated based on the column titles given previously. A histogram was made for the whoel dataset as well as each species. The only difference in the parameters was the removal
...
all pairs of variables as well as 4 distribution graphs of each individual variable. This was made using seaborn.pairplot with the parameter of hue = class. This automatically colours the plots based on the class column. It also autoamically includes
...
distributions that can be seen in the total dataset histograms. This is further seen in the distibution plots in the seaborn pairplot. When looking at the histograms and distribution graphs of the individual species, the data is quite normally distributed.
...
Other analyses done on the dataset was Pearson's correlation. Originally, individual Pearsons correlations were created using the numpy function .corrcoef. This would had to be done with every pair of variables with order not mattering. This was later commented out
...
the other variables are likely to be. This is primarily due to the measurements of the Setosa species. A quick look at the pairplot as well as the means will show that the Setosa species has the widest sepals but are much smaller in the other 3 varibales. The wider
a sepal is in the full dataset, the more likely it is to be the Setosa species, which would most likely mean smaller measurements in the other 3 varibles, hence the negative correlation. Aside from this negative correlation, other correlations are as would be
...
median and scatter plots of the dataset and individual species datapoints. This difference in groups explains some anomolies when looking at the data as a whole such as the negative correlation between sepal width and the other variables as well as the high skewness, 
...
the ease of pandas when making correlations and basic analyses very good. One technique that I had not done before was the scanning of the text file and writing to a new file in order to remove unneccesary lines. This is something that I will use a lot in the future I
would imagine. 
...
could also have researched into ways to create tables in the text file where all means, medians etc. among all species could be seen one one table, which would reduce the need to scroll up and down through the data when comparing analyses. Origincally, I wanted to add 
